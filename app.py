...
with tab2:
    ...
    if tienda_activa:
        ...
        if insumo_agregar == "➕ Crear Nuevo Insumo...":
            ...
            # Se quitó el bloqueo (disabled=True) para permitir usar otra unidad en la lista
            with col_ti3: t_uni = st.text_input("Unidad", value=datos_insumo["Unidad"], key=f"t_uni_edit_{insumo_agregar}")
        ...
        if st.button("➕ Agregar a la tienda"):
            if insumo_agregar == "➕ Crear Nuevo Insumo...":
                ...
                uni_final = t_uni.strip()
                cat_final = datos_insumo["Categoria"]

            # Filtro inteligente: Suma solo si coincide el nombre Y LA UNIDAD
            filtro_existente = (df_tienda['Ingrediente'] == insumo_final) & (df_tienda['Unidad'] == uni_final)
            if filtro_existente.any():
                idx = df_listas_global[(df_listas_global['Tienda'] == tienda_activa) & (df_listas_global['Ingrediente'] == insumo_final) & (df_listas_global['Unidad'] == uni_final)].index
...
with tab3:
    ...
    if modo_insumo == "➕ Crear Nuevo Insumo...":
        ...
        # Desbloqueado para permitir ajuste por receta
        with col_i3: unidad_insumo = st.text_input("Unidad", value=datos_insumo["Unidad"], key=f"e_uni_edit_{modo_insumo}")
    ...
    if not recetas_unicas:
        ...
        if cargar_edicion or ("receta_en_edicion" in st.session_state and st.session_state.receta_en_edicion == receta_mod):
            ...
            if modo_insumo_ed == "➕ Crear Nuevo Insumo...":
                ...
                # Desbloqueado para permitir ajuste
                with col_e3: ed_uni = st.text_input("Unidad", value=datos_insumo_ed["Unidad"], key=f"ed_e_uni_edit_{modo_insumo_ed}")
...
with tab5:
    ...
    st.info("Al modificar el nombre o categoría aquí, se actualizará automáticamente en **todas las recetas** e historiales. (Las unidades se manejan por separado en cada receta para permitir mayor flexibilidad).")
        ...
        if insumo_a_editar:
            ...
            if st.button("💾 Guardar Cambios en Todo el Sistema", type="primary"):
                if not nuevo_nombre_insumo.strip():
                    ...
                    # Ya NO se sobrescribe la unidad globalmente
